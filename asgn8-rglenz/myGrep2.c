...
int main(int argc, char *argv[]){
    int index=0,lineCount=0,wordCount=-1,tempLength=0;
    ...
    while(fgets(tmpLn,MAXLINE,file)){
        ...
        while(token!=NULL){ 
            ...
            if(!strcmp(token,argv[1])){
                ...
                else if(tail->lineNum==lineCount){
                    printf("%d, %d\n main",lineCount,tail->lineNum);
                    addRepeat(tail->nextR,wordCount);
                }
                else{
                    nodeCnt++;
                    tail=addToTail(tail,line,lineCount,wordCount);                      
